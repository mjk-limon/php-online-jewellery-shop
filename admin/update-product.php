...
<?php 
	if($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		if(!empty($color)) {
			...
			$color_count = count($color); $imageField	= "";
			for($i	= 0; $i < $color_count; $i++) {
				...
				if(!empty($_FILES[$each_color_name."pr_img"]['name'][0])){
					...
					for($j = 1; $j <= $each_color_count; $j++){
						$imageArray	= $j-1; $file = upload_image($each_color_name."pr_img", $key, "../proimg/{$prid}/");
						if($file !== false){rename($file, "../proimg/{$prid}/{$each_color_name}{$j}.jpg"); $uploadOk += 1;}
					}
				}
			}
		} else {
			...
			if(!empty($_FILES["pr_img"]['name'][0])) {
				...
				for($j = 1; $j <= $fields['images']; $j++){
					$imageArray	= $j-1; $file = upload_image("pr_img", $key, "../proimg/{$prid}/");
...
<div class="main-grid">
	<div class="panel panel-widget agile-validation">
		<div class="row my-div">
			<div class="col-md-12 my-div-heading"><h2>Update Product</h2></div>
			<div class="col-md-12">
