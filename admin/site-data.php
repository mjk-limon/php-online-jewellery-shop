...
switch ($dt) {
	case 'msg':
		$table_name = 'contact';
		$title = 'Mailbox';
		$sbis = array("All", "Unseen");
		$vdms = 'md';
		break;
	case 'odr':
		$table_name = 'p_order';
		$title = 'Orders';
		$sbis = array("All", "Unseen", "Processing", "Delivered", "Cancelled");
		$vdms = 'lg';
		break;
	case 'apr':
		$table_name = 'products';
		$title = 'All Products';
		$sbis = array("All", "Old uploaded", "Most viewed", "Discount", "Low Stock");
		$vdms = 'lg';
		break;
	case 'cls':
		$table_name = 'users';
		$title = 'Customer List';
		$sbis = array("All", "Old registered");
		$vdms = 'lg';
		break;
	case 'cpn':
		$table_name = 'coupons';
		$title = 'Coupons';
		$sbis = array("All", "Discount", "Used amount");
		$vdms = 'md';
		break;
	case 'nws':
		$table_name = 'newsletter';
		$title = 'Subscribers';
		$sbis = array("All", "Old subscribed");
		$vdms = 'md';
		break;
	case 'adm':
		$table_name = 'admins';
	...
	<div class="panel panel-widget">
		<div class="row my-div">
			...
			<div class="col-md-12">
				...
				<?php } else if ($dt == 'odr') { ?>
					...
					switch ($filter) {
						case 1:
							...
							break;
						case 2:
							$sort_sql = "admin_read='1'";
							break;
						case 3:
							$sort_sql = "admin_read='2'";
							break;
						case 4:
							$sort_sql = "admin_read='3'";
							break;
						default:
							$sort_sql = "1";
					}
							...
							?>
									<tr id="order-<?php echo $row_order['id']; ?>">
										...
										<td>
											...
											<p><strong> Shipment: </strong>&nbsp; <?php if ($row_order['shipment'] == null) {
																						echo "Normal";
																					} else {
																						echo $row_order['shipment'];
																					} ?></p>
											<p><strong> Payment: </strong>&nbsp; <?php $payment = $row_order['payment'];
																					if ($payment == 'rocket') {
																						echo 'Rocket';
											...
											<table class="table">
												...
												for ($i = 0; $i < count($o_products_id); $i++) {
													...
													$primg = "../proimg/" . $row_details['id'] . "/1.jpg";
												?>
													<tr>
														<td><img src="<?= $primg ?>" class="img-fluid img-thumbnail" style="max-height: 50px;" /></td>
														<td>
															<?= $row_details['name']; ?><br />
															<small>Size: <?= $o_products_size[$i] ?></small>
														</td>
														<td><?= $o_products_qty[$i] ?></td>
														<td><a href="../details.php?prid=<?= $o_products_id[$i] ?>" target="_blank"> View </a> </td>
													</tr>
												<?php } ?>
											</table>
										</td>
								...
								<tr>
									<td colspan="6" class="text-center bg-danger text-light">No data found!</td>
								</tr>
							<?php }
					...
					<script>
						$('.order-status').on('change', function() {
							...
							window.location = "?update_order_status=1&id=" + id + "&value=" + value;
						});
					</script>
				<?php } else if ($dt == 'apr') { ?>
					<!------ all products --------->
					...
					switch ($filter) {
						case 1:
							$sort_sql = "1 ORDER BY id ASC ";
							break;
						case 2:
							$sort_sql = "1 ORDER BY views DESC ";
							break;
						case 3:
							$sort_sql = "1 ORDER BY discount DESC ";
							break;
						case 4:
							$sort_sql = "1 ORDER BY item_left ASC ";
							break;
						default:
							$sort_sql = "1 ORDER BY id DESC";
					}
					?>
					<table class="table">
						<thead>
						...
						<tbody>
							...
							?>
									<tr>
										...
										<td>#<?= $row_pr['id'] ?></td>
										<td><?= $row_pr['name'] ?></td>
										<td>Tk <?= $row_pr['price']; ?></td>
										<td><?= $row_pr['category'] ?> - <?= $row_pr['subcategory'] ?></td>
										<td class="td-actions">
											<a href="<?= '../details.php?prid=' . restyle_url($row_pr['id']) ?>" target="_blank" class="btn btn-primary btn-sm">
				...
				<?php } ?>
				<nav>
					<ul class="pagination justify-content-end"><?= get_paging(get_total_rows($table_name, $sort_sql), 20, $page, "?dt=" . $dt . "&filter=" . $filter . "&") ?></ul>
				</nav>
			</div>
		</div>
	</div>
