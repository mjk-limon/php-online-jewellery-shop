...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (!empty($color)) {
		...
		$field_for_image = array();
		foreach ($color as $color_key => $each_color_name) {
			$field_for_image[] = count($_FILES[$each_color_name . "pr_img"]['name']);
			$total_image += count($_FILES[$each_color_name . "pr_img"]['name']);
			foreach ($_FILES[$each_color_name . "pr_img"]['name'] as $key => $value) {
				$j = $key + 1;
				$file = upload_image($each_color_name . "pr_img", $key, "../proimg/{$fields['id']}/");
				if ($file !== false) {
		...
		$total_image += $fields['images'];
		foreach ($_FILES["pr_img"]['name'] as $key => $value) {
			$j = $key + 1;
			$file = upload_image("pr_img", $key, "../proimg/{$fields['id']}/");
			if ($file !== false) {
	...
	if ($uploadOk == $total_image) {
		if ($conn->query($sql) == true) adminMessage('green', 'Successfully Uploaded Product');
										...
										?>
											<ul id="sm<?php echo $sub_i; ?>" class="sl-sub-ul" style="display: none;">
												...
												while ($row_sub = $result_sub->fetch_array()) {
													...
													$sub = $row_sub['sub'];
												?>
													<li class="sl-sub" data-sub="<?php echo htmlspecialchars($sub_header); ?>">
														<?php echo htmlspecialchars(ucwords($sub_header)); ?>
														<?php
														if (!empty($sub)) {
															echo " - " . htmlspecialchars(ucwords($sub));
														}
														?>
													</li>
												<?php
								...
								<div class="row">
									<div class="product-image">
										<div class="col-xs-12 col-sm-6 proimg-md" id="proimg1">
											...
											<input id="input-b0" name="pr_img[]" type="file" class="file" multiple required data-show-upload="false" data-max-file-size="2048" />
										</div>
										<div class="col-md-6 col-sm-12">
