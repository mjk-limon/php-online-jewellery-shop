		<div class="footer">
			<p>© <?= date("Y"); ?> . All Rights Reserved. Developed by <a><?php echo isset($myName) ? $myName : $companyName ?></a></p>
		</div>
		</section>
		<div class="site-msg">
		...
		<?php if (isset($_GET['smsg']) || isset($_GET['emsg'])) {
			$type = (isset($_GET['smsg'])) ? 1 : 0;
			switch ($type) {
				case 0:
					$msg = $_GET['emsg'];
					$label = 'danger';
			...
			<script>
				$(document).ready(function() {
					ds.showNotification('bottom', 'right', '<?= $label ?>', 'warning', "<?= $msg ?>")
				});
			</script>
		<?php } ?>
		</body>
