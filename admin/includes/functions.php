...
function get_paging($total_item, $item_per_page, $current_page, $target_url)
{
	...
	echo '"><a class="page-link" href="' . $target_url . 'page=' . ($current_page - 1) . '">Previous</a></li>';
	...
	echo '"><a class="page-link" href="' . $target_url . 'page=' . ($current_page + 1) . '">Next</a></li>';
}
...
{
	return get_some_data("procat", $extra_sql . " AND main='" . addslashes($main) . "' ORDER BY id ASC", "id,main,header,sub");
}
...
function get_title($page_name = 'index')
{
	...
	switch ($page_name) {
		case 'index':
			return get_single_index_data('site_settings', true, 'title');
			break;
		case 'product-page':
			global $main, $sub;
			return ucwords($main) . ' - ' . ucwords($sub) . ' || ' . $companyName;
			break;
		case 'details-page':
			global $product_name;
			return $product_name . ' - ' . $companyName;
			break;
		case 'inner-page':
			global $page_header;
			return $page_header . ' - ' . $companyName;
			break;
		case 'search-page':
			global $search;
...
function adminMessage($background, $html, $cv = false)
{
	...
	if ($cv) {
		foreach (explode(',', $cv) as $pv) $params[$pv] = null;
	}
...
function upload_image($imageName, $imageArray, $outputFolder)
{
	...
	if (isset($error_message) && strlen($error_message) > 0) {
		adminMessage('red', $error_message);
		return false;
	} else {
		if (move_uploaded_file($_FILES[$imageName]['tmp_name'][$imageArray], $target_path)) {
			...
			adminMessage("red", "Error uploading file !");
			return false;
		}
	}
}
...
function send_mail($email_from, $email_to, $email_subject, $messageBody)
{
	if (!isset($messageBody) || strlen($messageBody) <= 5) {
		echo 'Message content must be greater than 5 letter...';
		return false;
	} else {
